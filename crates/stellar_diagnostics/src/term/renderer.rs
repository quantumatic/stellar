...
impl<'w, 'c> Renderer<'w, 'c> {
    ...
    /// Empty line.
    pub(crate) fn render_empty(&mut self) -> Result<(), Error> {
        writeln!(self)?;
        Ok(())
    }
    ...
    /// ┌─ test:2:9
    /// ```
    pub(crate) fn render_snippet_start(
        &mut self,
        outer_padding: usize,
        locus: &Locus,
    ) -> Result<(), Error> {
    ...
    pub(crate) fn render_snippet_source(
        &mut self,
        outer_padding: usize,
        line_number: usize,
        source: &str,
        ...
        // ```
        {
            ...
            // Write inner gutter (with multi-line continuations on the left if necessastellar)
        ...
        if !single_labels.is_empty() {
                ...
                .chain(std::iter::once((placeholder_metrics, '\0')))
            {
                ...
                let current_label_style = single_labels
                    ...
                    .max_by_key(|&style| label_priority_key(style));

                // Update writer style if necessastellar
    ...
    /// │ │ │
    /// ```
    pub(crate) fn render_snippet_empty(
        &mut self,
        outer_padding: usize,
        severity: Severity,
        num_multi_labels: usize,
    ...
    /// · │ │
    /// ```
    pub(crate) fn render_snippet_break(
        &mut self,
        outer_padding: usize,
        severity: Severity,
        num_multi_labels: usize,
    ...
    ///      found type `String`
    /// ```
    pub(crate) fn render_snippet_note(
        &mut self,
        outer_padding: usize,
        message: &str,
    ) -> Result<(), Error> {
    ...
    /// Writes an inner gutter, with the left lines if necessastellar.
    fn inner_gutter(
        &mut self,
        severity: Severity,
        num_multi_labels: usize,
...
const fn label_priority_key(label_style: LabelStyle) -> u8 {
    match label_style {
